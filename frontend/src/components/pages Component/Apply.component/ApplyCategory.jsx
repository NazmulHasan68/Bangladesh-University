                ...
                {category.map((item, index) => (
                    <div key={index} className='flex gap-4 bg-white shadow-lg rounded-xl p-2'>
                        ...
                        <div>
                            ...
                            <p className='md:text-md text-xs font-medium py-1 line-clamp-2 md:line-clamp-3 '>{item.description}</p>
                            <Link to={'/applyInput'} className='md:px-6 md:py-2 px-4 py-1 text-xs rounded-full hover:bg-red-500 bg-red-400 text-white ml-[50%] md:ml-[75%]'>Apply</Link>
                        </div>
                    </div>
                ))}
            </div>
